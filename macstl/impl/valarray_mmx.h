		...
		namespace impl
			{
				...
				template <> struct chunk <long long>						{ typedef macstl::vec <long long, 4> type; };
				//! on builds without AVX, this will use vec<double, 2>
				template <> struct chunk <double>							{ typedef macstl::vec <double, 4> type; };
#else
				//! on builds with AVX, this will use vec<unsigned long long, 4>
				template <> struct chunk <unsigned long long>				{ typedef macstl::vec <unsigned long long, 2> type; };
				//! on builds with AVX, this will use vec<long long, 4>
				template <> struct chunk <long long>						{ typedef macstl::vec <long long, 2> type; };
				//! on builds with AVX, this will use vec<double, 4>
