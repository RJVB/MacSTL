		...
		template <typename T> class refarray: public impl::array_term <T>
			{
				public:
					...
					refarray( stdext::valarray<T> &rhs )
						: impl::array_term<T>(rhs.begin(), rhs.size())
					{
					}
					//@}
					...
					refarray& operator= (const stdext::valarray<T>& other)
						{
							if (this != &other)
								impl::copy_array (*this, refarray<T>(other.begin(), other.size()));
							return *this;
						}
