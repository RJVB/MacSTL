...
// 				DEFINE_VALARRAY_UNARY_FUNCTION (sincos, sinecosine)
				...
				template <typename T, typename Enable = void> class array_term:
					public term <T, array_term <T> >, public chunker <array_term <T> >
					{
						public:
							...
							typedef array_term<T,Enable>	array_term_type;
							...
							// RJVB
							/// outputs the array to a stream
							template <typename CharT, typename Traits>
								friend std::basic_ostream<CharT, Traits>& operator <<(std::basic_ostream <CharT, Traits>& os, array_term_type& x)
								{
									if( os.good() ){
									  typename std::basic_ostream <CharT, Traits>::sentry opfx(os);
										if( opfx ){
										  std::basic_ostringstream<CharT, Traits> s;
										  const_iterator val = x.begin();
											s.flags(os.flags());
											s.imbue(os.getloc());
											s.precision(os.precision());
// 											s << "<" << typeid(array_term_type).name() << ">{" << *val++;
											s << "{" << *val++;
											for( ; val != x.end() ; ++val ){
												s << "," << *val;
											}
