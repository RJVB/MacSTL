...
namespace stdext
	{
		namespace impl
			{
			...
			#if defined(__MACH__)
				// on Mach-based systems i.e. Mac OS X, malloc always returns 16-byte aligned memory
				...
				// RJVB : arbitrary alignment versions:
				template <int alignment> inline void* allocate_aligned(std::size_t size)	{ return ::_mm_malloc (size,alignment); }
				template <int alignment> inline void deallocate_aligned(void* ptr)			{ ::_mm_free (ptr); }
