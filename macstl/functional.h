...
// 20120621: gcc-mp-47 has __CHAR_BIT__ but not CHAR_BIT:
...
namespace stdext
	{
		...
		template <typename T> struct round: public std::unary_function <T, T>
			{
				const T operator() (const T& lhs) const
					{
						return (const T) ::round (lhs);
					}
			};
		...
		template <typename T> struct sinecosine: public std::binary_function <T, T, T>
			{
				const T operator() (const T& lhs, T* c) const
					{
						using std::sin;
						using std::cos;
						*c= cos(lhs);
